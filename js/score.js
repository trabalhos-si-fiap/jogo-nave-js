...
export class Score {
    ...
    async stopedShipTimer(ship) {
        // enquanto posição == parado inicia um timer que a cada 3 segundo remove 5 pontos.
        let intervalId;
        if (ship.direction == 'parado') {
            intervalId = setInterval(() => { this.stopedShipTime += 1 }, 1000)
        } else {
    ...
    // reduzir pontos quando a nave ficar mais do que 3 segundos parado
    losePoints() {
    ...
    // aumentar os pontos a cada 5 segundos de sobrevivencia
    morePoints() {
